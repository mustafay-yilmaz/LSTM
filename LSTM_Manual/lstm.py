...
def initilaize_output_units(vocab_size,v):
    global output_units
...
def save_model(parameters,embeddings,J,P,A,isim,id_char,vocab_size):
    saved_model = {
    ...
    joblib.dump(saved_model,f"{isim}.joblib")
    print(f"Model '{isim}.joblib' ismiyle Başarıyla Kaydedilei")

def predict(parameters,embeddings,id_char,vocab_size,predict_size):
    ...
    for i in range(predict_size):
        ...
        while(char!='.'):
            ...
            #pred = np.random.choice(len(ot[0]),1,p=ot[0])[0]
